output$text1intro = renderPrint({ 
  ...
  cat(paste("<p>Uncertainty in",varfmt("R0"),"has a large impact on the prediction of the position and size of the first infection peak. When",varfmt("R0,",params1a$R0),"peak infections are high",varfmt("I",I0),"and come early",varfmt("t.",t0/365),"For an outbreak with a lower",varfmt("R0,",params1b$R0),"peak infections are low",varfmt("I",I1),"and happen later",varfmt("t.",t1/365),"A lower",varfmt("R0"),"also results in a lower cumulative burden. For",varfmt("R0,",params1a$R0),"the burden at",varfmt("t",tmax/365),"is ",varfmt("B",max(df1b$B),prec=2),"which is",varfmt("B",max(df1b$B)/Bmax,prec=1,percent=1),"of the burden with high",varfmt("R0,",params1a$R0),sprintf("(%s).",varfmt("Bmax",Bmax,prec=2)),"This is the 'flattening the curve' effect&mdash;achieved by a lower",varfmt("R0"),"either through mitigation or a more accurate assessment that lowers the estimate."))
  cat(paste("<p>Because the latency",varfmt("(sigma)"),"transmission",varfmt("(beta)"),"and recovery",varfmt("(gamma)"),"rates are typically much faster than rate of immunity loss",varfmt("(omega)"),"uncertainty in immunity duration does not have a substantial impact on the first peak of the outbreak. During this peak, only a small part of cases are reinfections. However, increasing immunity duration from short",varfmt("1/omega",params1a$id/365,unit="years",prec=2),"to long",varfmt("1/omega",params1c$id/365,unit="years",prec=2),"has a substantial effect on delaying (but not flattening) the second peak. With long immunity, the second peak at",varfmt("R0,",params1a$R0),"is delayed from",varfmt("t",t2/365),"to",varfmt("t",t2L/365),"and at",varfmt("R0,",params1b$R0),"it is delayed from",varfmt("t",t3/365),"to",varfmt("t.",t3L/365),"Longer immunity also lowers the cumulative burden, because it delays infection peaks. When immunity duration is long, the burden at",varfmt("t",tmax/365),"drops to",varfmt("B",max(df2a$B)/Bmax,prec=1,percent=1),"and",varfmt("B",max(df2b$B)/Bmax,prec=1,percent=1),"for high and low values of ",varfmt("R0,"),"respectively, when compared to",varfmt("Bmax.")))

    cat(paste("<p>The possibility of coinfections during the annual flue season make accurate assessment of",varfmt("R0"),"and immunity duration important&mdash;depending on the exact values, infection peaks may fall within the flu season."))
