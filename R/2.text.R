...
output$text2intro = renderPrint({ 
  ...
  cat(paste("<p>Uncertainty in the parameters of the model of an outbreak can confound forecasting and policy-making. In this example, we show how we can decide between two mitigation actions A1",sprintf("(%s)",str0),"and A2",sprintf("(%s)",str1),"in the face of two possible immunity duration models M1",sprintf("(short, %s)",varfmt("1/omega",paramsa$id/365,prec=2,unit="years")),"and M2",sprintf("(long, %s).",varfmt("1/omega",paramsc$id/365,prec=2,unit="years"),"of an outbreak with",varfmt("R0.",R0max,prec=1))))
  ...
  cat(paste("<p>If we cannot decide whether M1 or M2 are right, we would average the burdens across models for each action and pick one that had the lower value. We would pick",Ab,"because it has the lower average burden",varfmt("Bavg.",avgsb,prec=1,percent=1)))
  
  cat(paste("<p>Could we make a better choice if we knew whether M1 or M2 were correct? To determine the value of accurately knowing the immunity duration, we calculate the expected value of perfect information (EVPI). The EVPI is the difference between the optimum of burden averages across actions",sprintf("opt(%s,%s) = %s",varfmt(value=avgs[1],prec=1,percent=1),varfmt(value=avgs[2],prec=1,percent=1),varfmt(value=min(avgs),prec=1,percent=1)),"and the average of optimums across models",sprintf("avg(%s,%s) = %s.",varfmt(value=opts[1],prec=1,percent=1),varfmt(value=opts[2],prec=1,percent=1),varfmt(value=mean(opts),prec=1,percent=1)),"In our case",varfmt("EVPI.",evpi,prec=1,percent=1)))
 
  if(evpi==0) {
    cat(paste("<p>When the EVPI is zero, then we do not need to resolve uncertainty about immunity duration because one of the actions has a better outcome for models. In this case, we would always pick",sprintf("%s.",Ab)))
  }
  if(evpi>0) {
    cat(paste("<p>The fact that the EVPI is not zero suggests that our choice of",Ab,"we made under no knowledge about immunity wouldn't be ideal for both models."))
  }
