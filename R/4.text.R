...
output$text4intro = renderPrint({ 
  ...
  cat(paste("<p>The cumulative burden",varfmt("B"),"is an important metric in measuring the magnitude of an outbreak. It is calculated from the total number of cases (infections) from the start of the outbreak up to a certain time point (e.g. 4 years)."))
  cat(paste("If immunity wanes, individuals may be infected more than once within the time span over which the burden is calculated."))
  cat(paste("Here, we will normalize the burden to the scenario with the largest cumulative burden",varfmt("Bmax",Bmax,prec=2),sprintf("(%s, %s).",varfmt("R0",paramsa$R0,prec=2),varfmt("1/omega",paramsa$id/365,prec=2,unit="years"))))  
    
  cat(paste("<h4>key observations</h4>"))
  cat(paste("<p>The 4-year burden sharply rises when recurrent epidemic waves (spikes in the infected fraction) cross the 4-year time horizon at which the burden is calculated. The burden remains flat across values of",varfmt("R0"),"that have a low infected fraction at the 4-year mark."))
